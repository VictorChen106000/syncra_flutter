...
/// in the user's browser. Centralised so every caller validates the URL the
/// same way and never throws into the UI — callers show a snackbar on `false`.
class UrlOpener {
  const UrlOpener._();

  /// Launches [rawUrl] in an external application (the browser). Returns `false`
  /// — never throws — when the URL is empty, not a parseable `http`/`https`
  /// link, or the platform refuses to open it, so the caller can surface a
