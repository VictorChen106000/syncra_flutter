...
/// Full-screen preview of a resume the agent built from scratch (`build_resume`)
/// that has been rendered but not yet saved. Mirrors [TailoredPreviewPage]:
/// white paper card + [SfPdfViewer], with the save gated behind the user's
/// approval. "Save to my resumes" persists it as a manual base resume; "Keep
/// editing" returns to the chat to refine it.
///
/// Reads its [ResumeDraftBlock] reactively by [blockId] so the UI flips to a
/// "Saved" state in place once the save completes.
class ResumeDraftPreviewPage extends ConsumerStatefulWidget {
  const ResumeDraftPreviewPage({super.key, required this.blockId});

  final String blockId;

  @override
  ConsumerState<ResumeDraftPreviewPage> createState() =>
      _ResumeDraftPreviewPageState();
}

class _ResumeDraftPreviewPageState
    extends ConsumerState<ResumeDraftPreviewPage> {
  bool _saving = false;

  Future<void> _save() async {
    setState(() => _saving = true);
    await ref.read(agentChatProvider.notifier).saveBuiltResume(widget.blockId);
    if (!mounted) return;
    ...
    if (block != null && block.isSaved) {
      ...
      messenger
        ..clearSnackBars()
        ..showSnackBar(SnackBar(content: Text(block!.error!)));
    }
  }
  ...
  void _keepEditing() {
    ...
    messenger
      ...
      ..showSnackBar(
        const SnackBar(
          content: Text('Tell me what to change and I\'ll rebuild it.'),
        ),
      );
  }
                  ...
                  child: Container(
                    ...
                    child: bytes == null
                        ...
                        : SfPdfViewer.memory(
                            bytes,
                            key: ValueKey(
                              'draft-${widget.blockId}-${bytes.length}',
                            ),
                          ),
                  ),
                ),
